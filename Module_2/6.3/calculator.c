    ...
    *count = 0;
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (strstr(entry->d_name, ".so")) { 
            ...
            char func_name[256];
            strncpy(func_name, entry->d_name, sizeof(func_name));
            char* ext = strstr(func_name, ".so");
            if (ext) *ext = '\0';  

            operation_func func = (operation_func)dlsym(handle, func_name);
            if (!func) {
            ...
            operations[*count].name = strdup(func_name);
